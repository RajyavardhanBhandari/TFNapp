...
export async function getHomeContent(): Promise<HomeContent> {
 ...
 const empty= {items:[],pagination:{page:1,perPage:6,total:0,totalPages:0,hasNextPage:false}};
 const [latestPage,fundingPage,founderPage,startupPage,technologyPage,aiPage]=await Promise.all([getArticles({page:1,perPage:12}),fundingCategory?getArticles({page:1,perPage:6,categoryId:fundingCategory.id}):Promise.resolve(empty),founderCategory?getArticles({page:1,perPage:6,categoryId:founderCategory.id}):Promise.resolve(empty),startupCategory?getArticles({page:1,perPage:6,categoryId:startupCategory.id}):Promise.resolve(empty),technologyCategory?getArticles({page:1,perPage:6,categoryId:technologyCategory.id}):Promise.resolve(empty),aiCategory?getArticles({page:1,perPage:6,categoryId:aiCategory.id}):Promise.resolve(empty)]);
 const latest=uniqueArticles(latestPage.items); const result={latest,featured:latest[0],forYou:latest.slice(1,5),funding:uniqueArticles(fundingPage.items),founderStories:uniqueArticles(founderPage.items),startupStories:uniqueArticles(startupPage.items),technology:uniqueArticles(technologyPage.items),ai:uniqueArticles(aiPage.items),categories}; setCached(cacheKey,result); return result;
}
