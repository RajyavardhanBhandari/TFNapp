...
export default function QuickScreen() {
        ...
        onPanResponderRelease: (_, gesture) => {
          if (Math.abs(gesture.dx) >= threshold) {
            ...
            Animated.timing(position, {
              toValue: {
                x: direction === 'right' ? width * 1.3 : -width * 1.3,
                y: gesture.dy,
              },
              duration: 220,
              useNativeDriver: true,
            }).start(() => void commit(direction));
          } else {
  ...
  const undo = async () => {
    ...
    if (lastAction.signalId) {
      await undoQuickSignal(lastAction.signalId);
    }
    ...
    <Screen scroll={false}>
      ...
      {loading ? (
        ...
        <View style={styles.center}>
          ...
          <Text style={{ color: theme.colors.mutedText }}>
            Explore more TFN stories soon.
          </Text>
          <Pressable
            onPress={() => void load()}
            style={[styles.button, { backgroundColor: theme.colors.text }]}
          >
            <Text style={{ color: theme.colors.inverseText }}>
              Refresh stories
            </Text>
          </Pressable>
        </View>
        ...
        <>
            ...
            >
              {current.featuredImage?.url ? (
                <Image
                  ...
                  accessibilityLabel={
                    current.featuredImage.alt || current.title
                  }
                  resizeMode="cover"
                  style={styles.image}
                />
              ...
              <View style={styles.cardCopy}>
                ...
                {current.excerpt ? (
                  <Text
                    numberOfLines={3}
                    style={[styles.excerpt, { color: theme.colors.mutedText }]}
                  >
                    {current.excerpt}
                  </Text>
                ) : null}
                <Pressable
                  ...
                  onPress={() => {
                    void recordQuickSignal(current.id, 'article_opened');
                    router.push({
                      pathname: '/article/[id]',
                ...
                >
                  <Text style={[styles.read, { color: theme.colors.text }]}>
          ...
          <View style={styles.actions}>
            <Pressable
              ...
              onPress={() => {
                Animated.timing(position, {
                  toValue: { x: -width * 1.3, y: 0 },
                  duration: 220,
                  useNativeDriver: true,
                }).start(() => void commit('left'));
              }}
              style={[styles.action, { borderColor: theme.colors.border }]}
            >
              <Text style={{ color: theme.colors.text }}>← Not for me</Text>
            </Pressable>
            ...
            >
              <Text style={{ color: theme.colors.text }}>↶ Undo</Text>
            </Pressable>
            ...
            <Pressable
              ...
              onPress={() => {
                Animated.timing(position, {
                  ...
                  duration: 220,
                  useNativeDriver: true,
                }).start(() => void commit('right'));
              }}
              style={[styles.action, { borderColor: theme.colors.border }]}
            >
              <Text style={{ color: theme.colors.text }}>Interested →</Text>
            </Pressable>
          </View>
        </>
      )}
...
const styles = StyleSheet.create({
  ...
  stack: { minHeight: 500, justifyContent: 'center' },
  card: {
    borderWidth: 1,
    borderRadius: 22,
    overflow: 'hidden',
  ...
  image: { width: '100%', height: 245 },
  cardCopy: { padding: 20, gap: 10 },
  category: {
    fontSize: 12,
    fontWeight: '800',
  ...
  read: { fontWeight: '800', marginTop: 4 },
  feedback: {
    position: 'absolute',
    top: 22,
    borderWidth: 2,
  ...
  feedbackText: { color: '#FFFFFF', fontWeight: '900', fontSize: 14 },
