...
export default function ProfileScreen() {
  ...
  if (!user) {
    return <Screen><AppHeader title="Profile" /><View style={styles.center}><View style={[styles.guestIcon, { backgroundColor: theme.colors.accentSoft, borderColor: theme.colors.border }]}><Initials name="The Founder Nation" color={theme.colors.accent} /></View><Text style={[styles.guestTitle, { color: theme.colors.text }]}>Your TFN account</Text><Text style={[styles.guestBody, { color: theme.colors.mutedText }]}>Sign in to manage your profile, save your preferences, and get a more relevant TFN experience.</Text><Pressable onPress={() => router.push('/auth/sign-in')} style={[styles.primaryButton, { backgroundColor: theme.colors.accent }]}><Text style={{ color: theme.colors.inverseText, fontWeight: '900' }}>Sign in</Text></Pressable><Pressable onPress={() => router.push('/auth/create-account')} style={[styles.secondaryButton, { borderColor: theme.colors.border }]}><Text style={{ color: theme.colors.text, fontWeight: '900' }}>Create account</Text></Pressable></View></Screen>;
  }
  ...
  return <Screen><AppHeader title="Profile" /><ScrollView contentContainerStyle={styles.wrap} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={refresh} />} showsVerticalScrollIndicator={false}>
    <Animated.View style={{ opacity: entrance, transform: [{ translateY }] }}>
      <View style={[styles.brandLine, { backgroundColor: theme.colors.accent }]} /><Text style={[styles.eyebrow, { color: theme.colors.mutedText }]}>THE FOUNDER NATION · MEMBER PROFILE</Text>
