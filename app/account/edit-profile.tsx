        ...
        >
          <Animated.View style={{ opacity: entrance, transform: [{ translateY }] }}>
            <View style={styles.intro}>
              ...
              <Text style={[styles.subheading, { color: theme.colors.mutedText }]}>Tell the founder ecosystem who you are and what you care about.</Text>
            </View>
            ...
            <View style={[styles.sectionCard, { backgroundColor: theme.colors.surface, borderColor: theme.colors.border }]}>
              ...
              <View style={styles.choices}>{PROFILE_ROLES.map((item) => <Choice key={item} label={item} selected={profile.role === item} onPress={() => update('role', item as ProfileRole)} colors={theme.colors} />)}</View>
            </View>
              ...
              {TEXT_FIELDS.map(([field, label]) => (
                <View key={field}>
                  ...
                  <TextInput
                    ...
                    placeholder={label}
                    placeholderTextColor={theme.colors.mutedText}
                    autoCapitalize={field.includes('url') ? 'none' : 'sentences'}
                    keyboardType={field === 'phone_number' ? 'phone-pad' : field.includes('url') ? 'url' : 'default'}
                    style={[styles.input, { color: theme.colors.text, borderColor: theme.colors.border, backgroundColor: theme.colors.background }]}
                  />
                </View>
              ))}
              ...
              <View style={styles.choices}>
                {PROFILE_INTERESTS.map((item) => (
                  <Choice
                    ...
                    onPress={() => update('interests', interests.includes(item) ? interests.filter((value) => value !== item) : [...interests, item])}
                    colors={theme.colors}
                  />
                ))}
              </View>
            ...
            <Pressable
              disabled={busy}
              onPress={save}
              style={({ pressed }) => [styles.saveButton, { backgroundColor: theme.colors.accent, opacity: busy ? 0.65 : 1, transform: [{ scale: pressed ? 0.98 : 1 }] }]}
            >
              <Text style={[styles.saveText, { color: theme.colors.inverseText }]}>{busy ? 'Saving changes…' : 'Save changes'}</Text>
            </Pressable>
            <Pressable onPress={() => router.back()} style={({ pressed }) => [styles.cancelButton, { opacity: pressed ? 0.65 : 1 }]}>
...
const styles = StyleSheet.create({
  flex: { flex: 1 },
  wrap: { width: '100%', maxWidth: 680, alignSelf: 'center', padding: 20, paddingBottom: 64 },
  loadingWrap: { padding: 24, alignItems: 'center' },
  loadingTitle: { fontSize: 20, fontWeight: '900' },
  loadingBody: { marginTop: 8, fontSize: 15 },
  intro: { marginBottom: 22 },
  eyebrow: { fontSize: 11, fontWeight: '900', letterSpacing: 1.4, marginBottom: 8 },
  heading: { fontSize: 30, lineHeight: 35, fontWeight: '900', letterSpacing: -0.7 },
  subheading: { fontSize: 15, lineHeight: 23, marginTop: 9, maxWidth: 540 },
  sectionCard: { borderWidth: 1, borderRadius: 22, padding: 18, marginBottom: 16 },
  sectionTitle: { fontSize: 20, fontWeight: '900' },
  sectionDescription: { fontSize: 14, lineHeight: 20, marginTop: 5, marginBottom: 12 },
  label: { fontSize: 13, fontWeight: '800', marginTop: 12, marginBottom: 7 },
  input: { minHeight: 52, borderWidth: 1, borderRadius: 14, paddingHorizontal: 15, fontSize: 16 },
  textarea: { minHeight: 118, borderWidth: 1, borderRadius: 14, padding: 15, fontSize: 16 },
  choices: { flexDirection: 'row', flexWrap: 'wrap', gap: 8 },
  choice: { borderWidth: 1, borderRadius: 18, paddingHorizontal: 13, paddingVertical: 10 },
  choiceText: { fontSize: 14, fontWeight: '800' },
  selectionHint: { fontSize: 13, marginTop: 13 },
  message: { fontSize: 14, fontWeight: '800', marginVertical: 8, lineHeight: 20 },
  saveButton: { minHeight: 54, borderRadius: 16, alignItems: 'center', justifyContent: 'center', marginTop: 8 },
  saveText: { fontSize: 16, fontWeight: '900' },
  cancelButton: { alignItems: 'center', justifyContent: 'center', paddingVertical: 18 },
